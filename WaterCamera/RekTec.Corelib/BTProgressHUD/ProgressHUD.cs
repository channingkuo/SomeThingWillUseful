...
namespace RekTec.Corelib.BTProgressHUD
{
    public class ProgressHUD : UIView
    {
        public ProgressHUD()
            : this((CGRect)UIScreen.MainScreen.Bounds)
        {
        }

        public ProgressHUD(CGRect frame)
            : base(frame)
        {
        ...
        public void SetOSSpecificLookAndFeel()
        {
            ...
            if (IsiOS7) {
                HudBackgroundColour = UIColor.White.ColorWithAlpha((nfloat)0.8f);
                HudForegroundColor = UIColor.FromWhiteAlpha((nfloat)0.0f, (nfloat)0.8f);
                HudStatusShadowColor = UIColor.FromWhiteAlpha((nfloat)200f / 255f, (nfloat)0.8f);
                _ringThickness = 1f;

            } else {
                HudBackgroundColour = UIColor.FromWhiteAlpha((nfloat)0.0f, (nfloat)0.8f);
        ...
        public UIColor HudBackgroundColour = UIColor.FromWhiteAlpha((nfloat)0.0f, (nfloat)0.8f);
        ...
        public UIFont HudFont = UIFont.BoldSystemFontOfSize((nfloat)16f);
        ...
        {
            ShowProgressWorker(progress, status, maskType, timeoutMs: timeoutMs);
        }
        ...
        {
            ShowProgressWorker(0, status, maskType, false, ToastPosition.Center, null, null, timeoutMs, true, img);
        }

        public void ShowContinuousProgressTest(string status = null, MaskType maskType = MaskType.None, double timeoutMs = 1000)
        {
            ShowProgressWorker(0, status, maskType, false, ToastPosition.Center, null, null, timeoutMs, true);
        }
        ...
        {
            ShowProgressWorker(status: status, textOnly: true, toastPosition: toastPosition, timeoutMs: timeoutMs, maskType: maskType);
        }

        public void SetStatus(string status)
        {
            SetStatusWorker(status);
        }
        ...
        {
            ShowImageWorker(image, status, TimeSpan.FromMilliseconds(timeoutMs));
        }
        ...
        public UIImage ErrorImage {
            get {
                return (IsiOS7 ? UIImage.FromFile("error_7.png") : UIImage.FromFile("error.png"));
            }
        }

        public UIImage SuccessImage {
            get {
                return (IsiOS7 ? UIImage.FromFile("success_7.png") : UIImage.FromFile("success.png"));
            }
        }
        ...
        public static ProgressHUD Shared {
            get {
                if (sharedHUD == null)
                    sharedHUD = new ProgressHUD((CGRect)UIScreen.MainScreen.Bounds);
                return sharedHUD;
            }
        }
        ...
        public override void Draw(CGRect rect)
        {
            using (var context = UIGraphics.GetCurrentContext()) {
                switch (_maskType) {
                    case MaskType.Black:
                        UIColor.FromWhiteAlpha((nfloat)0f, (nfloat)0.5f).SetColor();
                        context.FillRect((CGRect)Bounds);
                        break;
                    case MaskType.Gradient:
                        ...
                        using (var colorSpace = CGColorSpace.CreateDeviceRGB()) {
                            using (var gradient = new CGGradient(colorSpace, colors, locations)) {
                                ...
                                context.DrawRadialGradient((CGGradient)gradient, (CGPoint)center, (nfloat)0, (CGPoint)center, (nfloat)radius, (CGGradientDrawingOptions)CGGradientDrawingOptions.DrawsAfterEndLocation);
                            }
                        }
                                ...
                                double timeoutMs = 1000, bool showContinuousProgress = false, UIImage displayContinuousImage = null)
        {
            if (OverlayView.Superview == null) {
                ...
                foreach (UIWindow window in windows) {
                    if ((float)window.WindowLevel == (float)UIWindowLevel.Normal && !window.Hidden) {
...
(UIViewAnimationOptions)                    UIViewAnimationOptions.AllowUserInteraction | UIViewAnimationOptions.CurveEaseOut | UIViewAnimationOptions.BeginFromCurrentState,
(Action)                    delegate {
...
#else
            _fadeoutTimer = NSTimer.CreateTimer(duration, (Action<NSTimer>)DismissWorker);
#endif
...
#else
            _progressTimer = NSTimer.CreateRepeatingTimer(duration, (Action<NSTimer>)UpdateProgress);
#endif
        ...
        void UpdateProgress()
        {
            ...
            if ((float)RingLayer.StrokeEnd > 1) {
                RingLayer.StrokeEnd = 0.0f;
            } else {
        ...
        UIBezierPath CreateCirclePath(CGPoint center, float radius, int sampleCount)
        {
            ...
            smoothedPath.MoveTo((CGPoint)startPoint);
            ...
            for (int i = 1; i < sampleCount; i++) {
                ...
                smoothedPath.AddLineTo((CGPoint)point);
            }
            smoothedPath.AddLineTo((CGPoint)startPoint);
            return smoothedPath;
        }
        ...
        UIView OverlayView {
            get {
                if (_overlayView == null) {
                    _overlayView = new UIView((CGRect)UIScreen.MainScreen.Bounds);
        ...
        void DismissWorker()
        {
            ...
            UIView.Animate(0.3, 0, (UIViewAnimationOptions)UIViewAnimationOptions.CurveEaseIn | UIViewAnimationOptions.AllowUserInteraction,
(Action)                delegate {
        ...
        {
            HudView.Transform = CGAffineTransform.MakeRotation((nfloat)angle);
        ...
        void PositionHUD(NSNotification notification)
        {
            ...
            if (notification != null) {
                var keyboardFrame = (CGRect)UIKeyboard.FrameEndFromNotification((NSNotification)notification);
            ...
            CGRect orientationFrame = (CGRect)this.Window.Bounds;
            CGRect statusBarFrame = (CGRect)UIApplication.SharedApplication.StatusBarFrame;
            ...
            if (notification != null) {
                UIView.Animate(animationDuration,
                    0, (UIViewAnimationOptions)UIViewAnimationOptions.AllowUserInteraction, (Action)delegate {
