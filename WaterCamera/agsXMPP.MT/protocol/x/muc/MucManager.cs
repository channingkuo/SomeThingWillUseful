    ...
    public class MucManager
    {
        ...
        */
        ...
        public void ChangeNickname(Jid room, string newNick)
        {
            ...
            m_connection.Send(pres, to.Bare);
        }
        ...
        */
        ...
        public void AcceptDefaultConfiguration(Jid room, IqCB cb, object cbArgs, string roomDesc = "")
        {
            OwnerIq oIq = new agsXMPP.protocol.x.muc.iq.owner.OwnerIq(IqType.set, room);
            var submit = new Data(XDataFormType.submit);
            if (!string.IsNullOrWhiteSpace(roomDesc)) {
                var configField = new Field();
                configField.Var = "FORM_TYPE";
                configField.Type = FieldType.Hidden;
                configField.Value = "http://jabber.org/protocol/muc#roomconfig";
                submit.AddField(configField);

                var descField = new Field();
                descField.Var = "muc#roomconfig_roomdesc";
                descField.Type = FieldType.Text_Single;
                descField.Value = roomDesc;
                submit.AddField(descField);

                var nameField = new Field();
                nameField.Var = "muc#roomconfig_roomname";
                nameField.Type = FieldType.Text_Single;
                nameField.Value = roomDesc;
                submit.AddField(nameField);

            }

            oIq.Query.AddChild(submit);
            
            if (cb == null)
                m_connection.Send(oIq, oIq.Id);
            else
        ...
        */
        ...
        public void ChangeRole(Role role, Jid room, string nickname, string reason, IqCB cb, object cbArg)
        {
            AdminIq aIq = new AdminIq();
            aIq.To = room;
            aIq.Type = IqType.set;
            ...
            m_connection.IqGrabber.SendIq(aIq, cb, cbArg, false);
        }
